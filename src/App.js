...
export default function App() {
  const [isLightTheme, toggleTheme] = useReducer(
    (prevState) => !prevState,
    false
  );
  ...
  const [todoList, setTodoList] = useState(
    () => JSON.parse(localStorage.getItem("todoList")) || [createNewTodo()]
  );
  ...
  const handleRemove = (todoId = false) => {
    if (todoId) {
      ...
      setTodoList((prevTodoList) =>
        prevTodoList.filter((todo) => !todo.isCompleted)
      );
      setSelectAll(false);
    }
  };
  ...
  useEffect(() => {
    setTodoList((prevTodoList) =>
      prevTodoList.map((todo) => ({ ...todo, isCompleted: selectAll }))
    );
  }, [selectAll]);
  ...
  return (
    <DragDropContext onDragEnd={onDragEnd}>
      <div
        className={`container-fluid p-5 ${isLightTheme ? "light-theme" : ""}`}
      >
        ...
        <footer className="attribution">
          ...
          . Coded by <a href="https://github.com/alisariyer" target="_blank" rel="noreferrer">Ali Sariyer</a>.
        </footer>
      </div>
    </DragDropContext>
  );
